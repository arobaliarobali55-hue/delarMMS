...
const ProductList: React.FC = () => {
    ...
    useEffect(() => {
        ...
        // Refresh when stock changes
        const channel = supabase
            .channel('products_stock')
            .on('postgres_changes', { event: '*', schema: 'public', table: 'products' }, fetchProducts)
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') {
                    // console.log('Product subscription active');
                }
                if (status === 'CLOSED') {
                    console.warn('Product subscription closed unexpectedly');
                }
                if (status === 'CHANNEL_ERROR') {
                    console.error('Product subscription error:', status);
                }
            });
    ...
    return (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '24px' }}>
            {products.map((product) => (
                <div key={product.id} className="glass-panel" style={{ padding: '24px', display: 'flex', gap: '20px' }}>
                    <div style={{
